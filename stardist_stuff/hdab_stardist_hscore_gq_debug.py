...
# -*- coding: utf-8 -*-
"""
HDAB StarDist H-score (Global Quantiles) + Full Preprocessing Debug Visuals

This version adds saving of all preprocessing steps:
    hematoxylin/gray → CLAHE → median → normalized → final StarDist input.

Run example:
python hdab_stardist_hscore_gq_debug.py \
  --input /Users/jamieannemortel/Documents/IHC_dataset \
  --output /Users/jamieannemortel/Documents/IHC_out_v7 \
  --prob-thresh 0.6 --nms-thresh 0.25 --rescale 1.5 \
  --clahe --median-radius 1 \
  --vis --vis-input --vis-prob --vis-bounds --debug-hist
"""
...
def ensure_dirs(paths: Paths) -> Paths:
    ...
    return Paths(paths.input_dir, out, overlay, sbs)
...
def per_label_mean_with_ids(values: np.ndarray, labels: np.ndarray):
    props = measure.regionprops_table(labels, intensity_image=values,
...
def preprocess_for_stardist(rgb01: np.ndarray, seg_cfg: SegmentationConfig, debug: bool = False):
    stages = {}

    if seg_cfg.sd_input == "hematoxylin":
        im = hematoxylin_channel(rgb01)
        stages["hematoxylin"] = im
    else:
    ...
    if seg_cfg.sd_norm_percentiles is not None:
        ...
        stages["normalized"] = im

    im = np.clip(im, 0, 1).astype(np.float32)
    stages["final"] = im

    return (im, stages) if debug else im

def stardist_segment(rgb01, model, seg_cfg, debug: bool = False):
    if debug:
        sd_input, stages = preprocess_for_stardist(rgb01, seg_cfg, debug=True)
    else:
    ...
    if scale is not None:
        ...
        labels_small, _ = model.predict_instances(im_small, prob_thresh=seg_cfg.prob_thresh,
                                                  nms_thresh=seg_cfg.nms_thresh, n_tiles=seg_cfg.tile)
        ...
        labels, _ = model.predict_instances(sd_input, prob_thresh=seg_cfg.prob_thresh,
                                            nms_thresh=seg_cfg.nms_thresh, n_tiles=seg_cfg.tile)
        prob, _ = model.predict(sd_input)
        prob = prob.astype(np.float32)

    return labels.astype(np.int32), np.clip(prob, 0, 1), sd_input.astype(np.float32), stages
...
def colorize_probability(prob01: np.ndarray):
...
def overlay_on_rgb(rgb01: np.ndarray, heat_rgb01: np.ndarray, alpha: float):
    if heat_rgb01.shape[:2] != rgb01.shape[:2]:
        heat_rgb01 = transform.resize(heat_rgb01, rgb01.shape, order=1, anti_aliasing=True,
                                      preserve_range=True).astype(np.float32)
    return np.clip((1 - alpha) * rgb01 + alpha * heat_rgb01, 0, 1)

def render_boundaries_on_rgb(rgb01: np.ndarray, labels: np.ndarray):
    out = rgb01.copy()
...
def maybe_save_debug(
    ...
    stem: str,
    vis: DebugVisConfig,
    stages: Optional[Dict[str, np.ndarray]] = None
) -> None:
    if not vis.enable:
        return
    ...
    if vis.save_input:
        if stages is not None:
            for name, arr in stages.items():
                arr_viz = _rescale01(arr)
                skio.imsave(str(out_dir / f"{stem}_pre_{name}.png"),
                            _u8(np.repeat(arr_viz[..., None], 3, axis=-1)))

        # save final StarDist input
        skio.imsave(str(out_dir / f"{stem}_sdinput.png"),
                    _u8(np.repeat(sd_input01[..., None], 3, axis=-1)))
...
def compose_side_by_side(left: np.ndarray, right: np.ndarray) -> np.ndarray:
    if left.shape != right.shape:
        right = transform.resize(
            right, left.shape, order=1, anti_aliasing=True, preserve_range=True
        ).astype(left.dtype)
...
def run_global_quantile_pipeline(
    ...
    runtimes: List[float] = []
    image_mpx: List[float] = []

    n_vis = 0
    for p in imgs:
        rgb = read_image_rgb01(p)

        # ---- StarDist segmentation (NOTE: 4 returned values now) ----
        t0 = time()
        labels, prob01, sd_input01, stages = stardist_segment(
            rgb, model, seg_cfg, debug=(vis.enable and vis.save_input)
        )
        runtime = time() - t0
        ...
        # ---- Per-nucleus DAB OD from deconvolved D channel ----
        ...
        # ---- Optional debug saves (first N images if limit set) ----
        if vis.enable and (vis.max_images is None or n_vis < vis.max_images):
            maybe_save_debug(
                ...
                out_base=paths.output_dir, stem=p.stem, vis=vis, stages=stages
            )
            n_vis += 1
    ...
    for p, labels, means, ids, prob01, runtime, mpx in zip(
        ...
        row = {
            ...
            "runtime_sec": runtime,
            "runtime_s_per_mp": runtime / max(mpx, 1e-9),
...
def build_argparser() -> argparse.ArgumentParser:
    ap = argparse.ArgumentParser(description="Global-quantile HDAB H-score via StarDist2D (with full preprocessing debug visuals).")
    ...
    # Visualization
    ap.add_argument("--vis", action="store_true", help="Enable saving debug visuals (preproc/prob/bounds/input).")
    ap.add_argument("--vis-input", action="store_true", help="Save the StarDist input + all preprocessing stages.")
